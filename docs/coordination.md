...
* backend: исправлены окна (planned_end <= window_end), матрица без
  ...
  PUBLIC_TRANSPORT, география Подмосковья (geo-v2: Кашира/Домодедово/
  Ступино около своих городов), единый источник допуска
  (allowed_work_types), различимое оборудование, событие «инженер стал
  недоступен» — см. README;
* интерфейс: все 4 экрана работают; дата набора показывается вместо
  «Даты планирования»; план не строится при неразрешённых ошибках импорта;
  секция события «инженер стал недоступен» на экране результата;
  полный пользовательский сценарий проверяется автоматически
  (`tests/test_app_flow.py`, AppTest);
* результаты: план VALID, Assigned 81, baseline 29 (набор 17.08.2026,
  geo-v2). Прежние 97 назначений не использовать без пересчёта.